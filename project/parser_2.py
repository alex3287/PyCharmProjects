...
def seach_links(links):
    """Ищет все внутренние ссылки"""
    global links_list2, COUNT
    COUNT +=1
    links_list = []
    n = 0
    finde=0
    for i in links:
        ...
        if htm:
            n += 1
            print('сделано', n, 'проход', COUNT,'найдено', finde)
            links5 = all_links(htm)
            links6 = type_links(links5)
            links66 = sort2(links6[1])
            links7 = out_link(sort2(links66))
            for k in links7:
                if (k not in links_list2) and (k not in links_list):
                    links_list.append(k)
                    finde+=1
    links_list2 = links_list2 + sort2(links_list)

    if len(links_list):
            seach_links(links_list)
    return sort2(links_list2)

def seach_out_links(links):
    """Ищет все внешнии ссылки"""
    d=len(links)
    links_list = []
    n = 0
    for i in links:
        ...
        if htm:
            n += 1
            print('ищем внешнии ссылки, осталось проверить', d-n)
            links5 = all_links(htm)
            links6 = type_links(links5)
...
def main(url):
    ...
    input_links = seach_links(links3)
    print_l(seach_out_links(input_links))
    print('used')

if __name__ == "__main__":
    # url = input("Введите сайт для парсенга \n>>>")
    url = 'http://asschool3.ru'
    print(main(url))
